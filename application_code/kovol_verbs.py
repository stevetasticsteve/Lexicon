...
class KovolVerb:
    ...
    def create_tabulate_object(self):
        ...
        imperative = (self.sng_imperative, self.pl_imperative)
        return remote_past_tense, past_tense, future_tense, imperative


def read_verbsheet(
    spreadsheet=lexicon_config.settings["verb_spreadsheet"], output="class"
):
    ...
    raw_data = [
        x for x in raw_data if len(x) >= 5
    ]  # get rid rows lacking data (5 is where the Kovol words are)
    future1s_set = set(
        [
            (x[kovol_col], x[english_col])
            for x in raw_data
            if (x[actor_col] == "1s" and x[tense_col] == "future")
        ]
    )
    verbs = []
    if output == "class":
