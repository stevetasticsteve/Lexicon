...
def predict_paradigm(verb_tuple):
    ...
    data = [
        ["1s", remote_past[0], past[0], future[0], ""],
...
def display_paradigms(correct_paradigms, incorrect_paradigms):
    ...
    for p in correct_paradigms:
        print(p)
    print(
        "{n}/{m} correct: {p:.1f}% accuracy".format(
            n=len(correct_paradigms),
            m=predictions,
    ...
    for i, row in enumerate(predicted_data):
        for j, d in enumerate(row):
            if d != actual_data[i][j]:
                predicted_data[i][j] = "{red}{d}{white}".format(
                    red=red, d=d, white=white
                )
...
def process_verb_list(verbs):
    ...
    for v in verbs:
        ...
        p = format_paradigm(
            [
                {"title": "Predicted paradigm", "data": predicted_paradigm},
