...
class ReadLexiconTests(unittest.TestCase):
    ...
    def test_read_lexicon_blank_cell_response(self):
        ...
        with patch("tests.fixtures.settings", tests.fixtures.missing_cells):
            ...
            self.assertEqual(
                "", data[5]["phon"], "Blank phonetics not returning correctly"
            )
            ...
            self.assertEqual(
                "", data[1]["eng"], "Blank English not returning correctly"
            )
            # missing Tok Pisin - return ''
            self.assertEqual(
                "", data[1]["tpi"], "Blank Tok pisin not returning correctly"
            )
            # missing definition - return ''
            self.assertEqual(
                "", data[1]["def"], "Blank definition not returning correctly"
            )
            ...
            self.assertEqual(
                "",
                data[1]["trans"],
                "Blank example translation not returning correctly",
            )
            # missing date - return ''
            self.assertEqual("", data[1]["date"])
            # missing entered - return ''
            self.assertEqual(
                "", data[1]["enter"], "Blank entered by not returning correctly"
            )
            # missing check, syn, ant, see also, tag - return ''
            self.assertEqual(
                "", data[1]["syn"], "Blank synonym not returning correctly"
            )
            self.assertEqual(
                "", data[1]["ant"], "Blank antonym not returning correctly"
            )
            self.assertEqual(
                "", data[1]["check"], "Blank check not returning correctly"
            )
            self.assertEqual(
                "", data[1]["link"], "Blank see also not returning correctly"
            )
    ...
    def test_read_lexicon_no_blank_senses_in_return(self):
        ...
        for row in data:
            self.assertIsNot(
                row["sense"], "", "Blank sense numbers are in the return value"
            )
            self.assertIs(
                int, type(row["sense"]), "Sense number for row isn't an integer"
            )
    ...
    def test_read_lexicon_no_blank_id_in_return(self):
        def check_ids(d):
            for row in d:
                self.assertIsNot(
                    row["id"], "", "Blank ID numbers are in the return value"
                )
                self.assertIs(
                    int, type(row["id"]), "ID number for row isn't an integer"
                )
    ...
    def test_read_lexicon_data_not_as_expected(self):
        ...
        with patch("tests.fixtures.settings", tests.fixtures.dodgy_data):
            ...
            for k, v in data[0].items():
                if k == "id":
                    self.assertEqual(
                        type(v), int, "value for {k} is not a int".format(k=k)
                    )
                elif k == "sense":
                    self.assertEqual(
                        type(v), int, "value for {k} is not a int".format(k=k)
                    ...
                    self.assertEqual(
                        type(v),
                        datetime.date,
                        "value for {k} is not a datetime".format(k=k),
                    )
                else:
                    self.assertEqual(
                        type(v), str, "value for {k} is not a string".format(k=k)
                    )
...
class SupportingFunctionsTests(unittest.TestCase):
    ...
    def test_check_config(self):
        ...
        with self.assertRaises(FileNotFoundError) as error:
            read_data.check_settings(config_file=settings)
        self.assertIn(
            "The following file doesn't exist: {t}".format(t=settings["target_folder"]),
            str(error.exception),
            "Error message wrong",
        )
    ...
    def test_letter_to_number(self):
        # valid input
        self.assertEqual(
            read_data.letter_to_number("A"), 0, "Letter to number giving wrong output"
        )
        self.assertEqual(
            read_data.letter_to_number("B"), 1, "Letter to number giving wrong output"
        )
        self.assertEqual(
            read_data.letter_to_number("a"), 0, "Letter to number giving wrong output"
        )
        self.assertEqual(
            read_data.letter_to_number("Z"), 25, "Letter to number giving wrong output"
        )
        # invalid input
