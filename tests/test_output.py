...
class HTMLGenerationTests(unittest.TestCase):
    ...
    def test_generate_html(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_html(tests.fixtures.good_processed_data)
    ...
    def test_generate_lexicon_page_exists(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_lexicon_page(tests.fixtures.good_processed_data, None)
    ...
    def test_generate_lexicon_page_contents(self):
        ...
        with open(self.lex_page, 'r') as file:
            ...
            for word in tests.fixtures.good_processed_data:
    ...
    def test_generate_error_page_with_repeated_sense_errors(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_html(tests.fixtures.repeated_sense_processed_data)
    ...
    def test_generate_error_page_with_pos_errors(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_html(tests.fixtures.missing_pos_processed_data)
        self.assertTrue(os.path.exists(self.error_page))
        with open(self.error_page, 'r') as file:
            file = file.read()
    ...
    def test_error_page_multiple_errors(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_html(tests.fixtures.multiple_error_processed_data)
        self.assertTrue(os.path.exists(self.error_page))
        with open(self.error_page, 'r') as file:
            file = file.read()
            self.assertIn('<h3>Part of speech missing</h3>', file, 'Error heading missing')
    ...
    def test_generate_Eng_page_exists(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_eng_page(tests.fixtures.good_processed_data)
    ...
    def test_generate_check_page_exists(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.generate_check_page(tests.fixtures.good_processed_data)
...
class OtherFileGenerationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.paDb_path = os.path.join(parent_folder, 'test_output', '{language}_phonology_assistant.db'.format(
            language=tests.fixtures.settings['language']))
    ...
    def test_create_phonemic_assistant_new_file_exists(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.create_phonemic_assistant_db(tests.fixtures.good_processed_data, checked_only=False)
        self.assertTrue(os.path.exists(self.paDb_path))
        with patch('lexicon_config.settings', tests.fixtures.settings):
            with self.assertRaises(AssertionError) as error:
                output.create_phonemic_assistant_db(tests.fixtures.good_processed_data, checked_only=True)
...
"""
    ...
    def test_create_dataset_csv(self):
        with patch('lexicon_config.settings', tests.fixtures.settings):
            output.create_dataset_csv(tests.fixtures.good_processed_data)
